...
class TraDE(nn.Module):
    ...
    def samples(self, batch_size):
        samples = torch.zeros(batch_size, self.n, device=self.device,
                              dtype=torch.float64)  # torch.randint(0, 2, size=(batch_size, self.n), dtype=torch.float64, device=self.device)
